<template>
  <div class="ww-input-basic" :class="{ editing: isEditing }">
    <input
      ...
      :placeholder="
        isAdvancedPlaceholder ? '' : wwLang.getText(content.placeholder)
      "
      :style="style"
      :min="content.min"
      :max="content.max"
    ...
    >
      <wwElement
        ...
        :ww-props="{
          text: wwLang.getText(content.placeholder) || 'Placeholder',
        }"
      ></wwElement>
    </div>
  </div>
...
export default {
  ...
  setup(props) {
    ...
    const step = computed(() => {
      return type.value === "decimal" || type.value === "number"
        ? props.content.step
        : 1;
    });
    function formatValue(value) {
      ...
      const length =
        value.indexOf(".") !== -1
          ? props.content.precision.split(".")[1].length
          : 0;
      const newValue = parseFloat(
        Number(value).toFixed(length).replace(",", ".")
      );
      return newValue;
    }
    ...
    const { value: variableValue, setValue } =
      wwLib.wwVariable.useComponentVariable({
        ...
        name: "value",
        type: computed(() =>
          ["decimal", "number"].includes(type.value) ? "number" : "string"
        ),
        defaultValue: computed(() =>
          props.content.value === undefined
            ? ""
            : formatValue(props.content.value)
        ),
      });
  ...
  computed: {
    ...
    placeholderSyle() {
      const transition = `all ${
        this.noTransition ? "0ms" : this.content.transition
      ...
      if (this.content.forceAnimation && this.isEditing)
        return animatedPosition;
      if (this.value && this.value !== 0) return animatedPosition;
      if (this.isDebouncing) return animatedPosition;
      if (this.content.animationTrigger === "focus" && this.isFocused)
        return animatedPosition;
